...
void StackCircleCollection(float xc, float yc, float radius, int nestingLength)
{
    ...
    while (!IsEmpty()) // цикл пока стек не пуст
    {
        ...
        RectangleShape rectangle(Vector2f(radius,radius)); // создаем окружность
        ...
        rectangle.setPosition(point.x + point.radius / 2, point.y + point.radius / 2); // перемещаем окружность в точку xc, yc
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        StackCircleCollection(400, 400, 150, 2);
        // Отрисовка окна
        window.display();
    }
    }